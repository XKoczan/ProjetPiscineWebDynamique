...
if(isset($_POST['test'])){
    ...
    for($i=0; $i<$nbmodele; $i++)
    {
        ...
        if($nbmodele == 1)
        {
            $sql = "INSERT INTO cataloguemodele (prix, idprod, photos, video, description, stock, nbvente)
            VALUES ('".$_POST[$i."prix"]."', ".$id_article.", '".$photo."', '".$video."', '".$_POST[$i.'Description']."', ".$_POST[$i.'stock'].", 0)";
        }else{
            $sql = "INSERT INTO cataloguemodele (nom, prix, idprod, photos, video, description, stock, nbvente)
